...
## keep SNPs from list
keepIndex = which((snpMap$pos_hg19 %in% riskLoci$hg19POS) & (snpMap$pos_hg19 %in% snpMap_hippo$pos_hg19))	# keep 6350 snps
...
save(meGene, meExon, meJxn, meTx,
	file="eqtl_tables/matrixEqtl_output_dg_raggr_4features.rda")
...
allEqtl$gencodeTx = CharacterList(c(as.list(rowRanges(rse_gene)$gencodeTx[match(geneEqtl$gene, 
	...
	as.list(txEqtl$gene)))
save(allEqtl, file="eqtl_tables/mergedEqtl_output_dg_raggr_4features.rda",compress=TRUE)
