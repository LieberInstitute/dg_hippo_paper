...
raceIndex = which(!riskLoci$Population %in% c("ACB+ASW+ESN+GWD+LWK+MSL+YRI","CEU+FIN+GBR+IBS+TSI"))
## keep SNPs from list
keepIndex = which(snpMap$hg19POS %in% riskLoci$hg19POS2)	# keep 10,777 snps from snpMap
snpMap = snpMap[keepIndex,]
keepIndex = which(riskLoci$hg19POS2 %in% snpMap$hg19POS & riskLoci$Population %in% c("ACB+ASW+ESN+GWD+LWK+MSL+YRI","CEU+FIN+GBR+IBS+TSI"))	# keep 10,754 snps from riskLoci
...
# riskLoci_full$SNP2_missing[raceIndex] = "other race"
...
dlp = read.csv("/dcl01/lieber/ajaffe/lab/brainseq_phase2/eQTL_GWAS_riskSNPs/raggr_179snps_dlp_eqtls_fdr05.csv", row.names=1)

## unique SNPs
length(unique(dg$hg19POS))
length(unique(hippo$hg19POS))
length(unique(dlp$hg19POS))
v = venn.diagram(list(DG = dg$hg19POS, HIPPO = hippo$hg19POS, DLPFC = dlp$hg19POS), 
	fill = c("darksalmon", "lightgoldenrod", "lightsteelblue2"), main="", main.pos = c(.5, .05), cat.cex = 1.9, cex=3,
	margin = .1, filename = NULL)
pdf("venn_unique_SNP_with_dlpfc.pdf")
grid.draw(v)
dev.off()
...
v = venn.diagram(list(DG = unique(dg$hg19POS[dg$hg19POS %in% riskLoci$hg19POS1]), 
	...
	fill = c("darksalmon", "lightgoldenrod", "lightsteelblue2"), main="", main.pos = c(.5, .05), cat.cex = 1.9, cex=3,
	margin = .1, filename = NULL)
pdf("venn_unique_indexsnp_with_dlpfc.pdf")
grid.draw(v)
dev.off()
...
v = venn.diagram(list(DG = dg$gene, HIPPO = hippo$gene, DLPFC = dlp$gene), 
	fill = c("darksalmon", "lightgoldenrod", "lightsteelblue2"), main="", main.pos = c(.5, .05), cat.cex = 1.9, cex=3,
	margin = .1, filename = NULL)
pdf("venn_unique_features_with_dlpfc.pdf")
grid.draw(v)
dev.off()
...
v = venn.diagram(list(DG = paste0(dg$SNP, dg$gene), 
	...
	fill = c("darksalmon", "lightgoldenrod", "lightsteelblue2"), main="", main.pos = c(.5, .05), cat.cex = 1.9, cex=3,
	margin = .1, filename = NULL)
pdf("venn_unique_snpfeatpairs_with_dlpfc.pdf")
grid.draw(v)
dev.off()
...
v = venn.diagram(list(DG = dg$Symbol[!is.na(dg$Symbol)], 
				HIPPO = hippo$Symbol[!is.na(hippo$Symbol)],
				DLPFC = dlp$Symbol[!is.na(dlp$Symbol)]), 
	fill = c("darksalmon", "lightgoldenrod", "lightsteelblue2"), main="", main.pos = c(.5, .05), cat.cex = 1.9, cex=3,
	margin = .1, filename = NULL)
pdf("venn_unique_symbol_with_dlpfc.pdf")
grid.draw(v)
dev.off()
...
proxInd = match(region$hg19POS, riskLoci_full$hg19POS2)
region$IndexSNP = riskLoci_full$SNP1_Name[proxInd]
region$IndexSNP_hg19POS = riskLoci_full$hg19POS1[proxInd]
...
write.csv(dlp, file="raggr_179_snps_dlpfc_eqtls_fdr05.csv")
